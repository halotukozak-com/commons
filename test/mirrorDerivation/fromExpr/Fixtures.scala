...
object Macro:
  ...
  def matchTuple2Impl(using Quotes): Expr[Boolean] =
    Expr(tuple2FromExpr.unapply('{ (1, "x") }).contains((1, "x")))
  ...
  // default implicit-search budget; the search still recovers via the flat tupleN
  // instance once it gives up on the recursive one, but each abandoned attempt
  // triggers a (non-fatal upstream) E168 warning that -Werror would otherwise fail on.
  @nowarn("msg=Implicit search problem too large")
