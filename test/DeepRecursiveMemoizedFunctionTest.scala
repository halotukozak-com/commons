...
object DeepRecursiveMemoizedFunctionTest:
  def deepSum(n: Int): Int = deepRecursiveMemoized:
    if n == 0 then 0
    else 1 + deepSum(n - 1)
  ...
  def deepSumAcc(n: Int, acc: Long): Long = deepRecursiveMemoized:
    if n == 0 then acc
    else deepSumAcc(n - 1, acc + n)

  // recursive call in the last expression of a block, after a local val
  def deepBlockSum(n: Int): Int = deepRecursiveMemoized:
    if n == 0 then 0
    else
      val prev = n - 1
      1 + deepBlockSum(prev)
  ...
  def deepWithBump(n: Int)(using bump: Bump): Int = deepRecursiveMemoized:
  ...
  def deepSumWith[A: Numeric](n: Int, a: A): A = deepRecursiveMemoized:
    if n == 0 then a
  ...
  def deepImplicitStep(n: Int)(implicit step: Step): Int = deepRecursiveMemoized:
...
class DeepRecursiveMemoizedFunctionTest extends munit.FunSuite:
  import DeepRecursiveMemoizedFunctionTest.*
  ...
  test("memoizes so repeated overlapping subcalls within one invocation are computed only once") {
    var calls = 0
    def countedFib(n: Int): Int = deepRecursiveMemoized:
      calls += 1
      if n < 2 then n
      else countedFib(n - 1) + countedFib(n - 2)

    assertEquals(countedFib(30), plainFib(30))
    // naive (non-memoized) fib(30) makes over a million calls; memoization bounds it to
    // one compute per distinct argument (0..30).
  ...
  test("does not memoize across separate top-level invocations") {
    ...
    def countedDouble(n: Int): Int = deepRecursiveMemoized:
      calls += 1
      if n == 0 then 0 else 2 + countedDouble(n - 1)

    assertEquals(countedDouble(5), 10)
  ...
  test("rejects a recursive call nested inside a try instead of silently looping forever") {
    val res = compileErrors(
      """def f(n: Int): Int = deepRecursiveMemoized { try { if n == 0 then 0 else 1 + f(n - 1) } catch case _: Exception => -1 } """,
    )
    assert(res.contains("cannot safely trampoline"), res)
  }

  test("rejects deepRecursiveMemoized used inside a lambda instead of silently skipping the trampoline") {
    val res = compileErrors(
      """lazy val f: Int => Int = n => deepRecursiveMemoized { if n == 0 then 0 else 1 + f(n - 1) }""",
    )
